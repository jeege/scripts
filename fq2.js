...
function get_flat_2(itemId, token, ua) {
    ...
    const flat_2_in = String.fromCharCode.apply(null, flat_arr.concat(flat_end))
...
function get_a_bogus(itemId, token, ua) {
    ...
    function getContent(itemId) {
        ...
        // console.log(itemId, token, randomUA, a_bogus)
        return {
    ...
    // getContent('7094460813142196744')
})()
}
const Fq = Fanqie()
const {url, headers} = Fq.getContent('7094460813142196744')
fetch(url, {headers})
.then(res => res.json())
.then(data => {
    console.log(Fq.decodeContent(data.data.chapterData.content))
})